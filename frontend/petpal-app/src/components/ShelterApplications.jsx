...
const ShelterApplications = () => {
    ...
    const endpoint = `http://127.0.0.1:8000/applications/${userId}/shelter/list/`;
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            console.log(result);
            const filtered = result.filter((pet) => pet.pet_id === parseInt(petId));
                ...
                {filteredData.map((input) => (
                    <form key={input.id} className="form-container" encType="multipart/form-data">
                        <div style={{ marginBottom: "5px" }} className="form-group">
                            <label htmlFor="username" className="label-box">User ID</label>
                            <input type="text" className="form-control input-box" id="username" name="user_id" value={input.user_id} readOnly={true}/>
                        </div>
                        <div style={{ marginBottom: "5px" }} className="form-group">
                        ...
                        <a style={{ marginBottom: "10px" }} href={`/shelterupdate/${input.id}`} type="button" className="btn btn-primary">Update Status</a>
                    </form>
                ))}
            </div>
        </div>
